...
{
  I2C_readBytes(slaveAddr, reg, length, data,0);
}

void ist_i2c_write(uint8_t slaveAddr, uint8_t reg, uint8_t data)
{
  I2C_writeByte(slaveAddr, reg, data);
}
bool ist8310TestConnection()
{
  uint8_t buffer;
...
bool ist8310_Init(){
    ...
    DelayMsec(50);  //sensor initial time : 50 milliseconds
    
    connect = ist8310TestConnection();
