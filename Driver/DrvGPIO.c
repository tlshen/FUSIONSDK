...
int32_t DrvGPIO_EnableInt(E_DRVGPIO_PORT port, int32_t i32Bit, E_DRVGPIO_INT_TYPE TriggerType, E_DRVGPIO_INT_MODE Mode)
{
    ...
    {
        NVIC_SetPriority(GPAB_IRQn, (1<<__NVIC_PRIO_BITS) - 1);
        NVIC_EnableIRQ(GPAB_IRQn);
    }

    if ((port == E_GPC) || (port == E_GPD) || (port == E_GPE) || (port == E_GPF))
    {
...
void DrvGPIO_EnableINT0(E_DRVGPIO_INT_TYPE TriggerType, E_DRVGPIO_INT_MODE Mode, GPIO_EINT0_CALLBACK pfEINT0Callback)
{
    volatile uint32_t u32Reg;
...
{  
	switch ( function )
	{
		...
		case E_FUNC_SPI2:	
		{
			/* Modify by Stanley 20120725 : for NUC105 SPI2 */
          	/*outpw(&SYS->GPDMFP, inpw(&SYS->GPDMFP) | (0xF<<0));
          	
           	outpw(&SYS->GPAMFP, inpw(&SYS->GPAMFP) | (0x1<<9));
            outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) | (0x1<<7));*/
			outpw(&SYS->GPDMFP, inpw(&SYS->GPDMFP) | (0x0<<0));
			*((volatile unsigned int *)(0x50000054)) = 0x000F0000;
		}break;				
      ...
      outpw(&SYS->ALTMFP1, inpw(&SYS->ALTMFP1) | (0x3<<(18+(function-E_FUNC_ADC0))));

		case E_FUNC_ADC0:
			outpw(&SYS->ALTMFP1, inpw(&SYS->ALTMFP1) | (0x1<<(16+(function-E_FUNC_ADC0))));
		case E_FUNC_ADC1:
		case E_FUNC_ADC2:
		case E_FUNC_ADC3:
		case E_FUNC_ADC4:
		...
		{
         outpw(&SYS->GPDMFP, inpw(&SYS->GPDMFP) | (0x1<<(function-E_FUNC_ADC0)));
    }
		break;
		...
		case E_FUNC_PS2:
		{
          	outpw(&SYS->GPFMFP, inpw(&SYS->GPFMFP) | (0x3<<2));
			outpw(&SYS->ALTMFP1, inpw(&SYS->ALTMFP1) & (0xf0ffffff));
        }break;
